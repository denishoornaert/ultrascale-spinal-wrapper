...
case class ClockFrequencyChecker() extends KV260(
  config = new KV260Config(
    withPL_CLK0 = PLL.IO(333 MHz),
    withLPD_HPM0 = true
  )
) {
...
object ClockFrequencyCheckerVerilog extends App {
  Config.spinal.generateVerilog(ClockFrequencyChecker())
}

object ClockFrequencyCheckerVhdl extends App {
  Config.spinal.generateVhdl(ClockFrequencyChecker())
}
